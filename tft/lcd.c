...
u16 LCD_RD_DATA(void)
{
	...
	LCD_RD_CLR;
	MB_USLEEP(1);
	if(lcddev.id==0X8989) MB_USLEEP(2);//FOR 8989,延时2us
	da=LCD_DATA_I;
	LCD_RD_SET;
	LCD_CS_SET;
...
void LCD_Init(void)
{
	...
	MB_Sleep(50); // delay 50 ms
  	lcddev.id = LCD_ReadReg(0x0000);
  	printf("s1:%x\n",lcddev.id);

	if(1)		//也不是NT35310,尝试看看是不是NT35510
	{
		LCD_WR_REG(0XDA00);
		LCD_RD_DATA();   		//读回0X00
		LCD_WR_REG(0XDB00);
...
{
	LCD_ShowString(x, y, 800, 480, 24, p);
}
//在指定位置画一个指定大小的正置等边三角形
//(x,y):左顶角
//d    :边长
void LCD_DrawTriangle_pos(u16 x,u16 y,u16 d)
{
	LCD_DrawLine(x, y, x+d, y);								//底边
	LCD_DrawLine(x, y, x+(d*0.5f), y-(d*0.866f));			//左斜边
	LCD_DrawLine(x+(d*0.5f), y-(d*0.866f),x+d,y);			//右斜边
}

//在指定位置画一个指定大小的反置等边三角形
//(x,y):底部顶点
//d    :边长
void LCD_DrawTriangle_neg(u16 x,u16 y,u16 d)
{
	LCD_DrawLine(x, y, x-(d*0.5f), y-(d*0.866f));								//底边
	LCD_DrawLine(x, y, x+(d*0.5f), y-(d*0.866f));							//左斜边
	LCD_DrawLine(x-(d*0.5f),y-(d*0.866f), x+(d*0.5f),y-(d*0.866f));			//右斜边
}
...
void LCD_CalDispaly()
{
	int i;
	POINT_COLOR=BLUE;
	LCD_DrawLine(0, 400, 800, 400);
	for(i=133;i<800;i+=133)
	{
		LCD_DrawLine(i, 400, i, 480);
	}
	POINT_COLOR=YELLOW;
	for(i=41;i<800;i+=266)
	{
		LCD_DrawTriangle_pos(i,470,50);
	}
	for(i=199;i<800;i+=266)
	{
		LCD_DrawTriangle_neg(i,470,50);
	}
	POINT_COLOR=BLACK;
	LCD_showString(0,370,"YScale");
	LCD_showString(266,370,"XScale");
	LCD_showString(532,370,"Level");
}
