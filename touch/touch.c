...
u8 TP_Read_XY(u16 *x,u16 *y)
{
	...
	GT9147_Scan(&tempy,&tempx);
	//printf("tp_dev.x[t]=%d\t",x);
	tempx=480-tempx;
	//*x=tempx;
	//*y=tempy;
...
u8 TP_Scan(u8 tp)
{
	if(PEN==0)//有按键按下
	{
		...
		else if(GT9147_Scan(&tp_dev.x[0],&tp_dev.y[0]))//读取屏幕坐标
		{
	 		tp_dev.x[0]=tp_dev.xfac*tp_dev.x[0]+tp_dev.xoff;//将结果转换为屏幕坐标
			tp_dev.y[0]=tp_dev.yfac*tp_dev.y[0]+tp_dev.yoff;
			printf("tp_dev.x[0]=%d\t",tp_dev.x[0]);
			printf("tp_dev.y[0]=%d\n",tp_dev.x[0]);
...
u8 TP_judgment(int x,int y)
{
	//if(y<400 && y>480)return 0;
	if(y>400)
	{
		//这里吧2，1顺序颠倒，因为增减的是右移运算符的值
		if(  x<133)
		{
			return 2;
